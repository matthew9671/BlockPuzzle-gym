...
class ImitationRollout:
    ...
    def __init__(self, make_env, policy, expert, dims, logger, T, rollout_batch_size=1,
        ...
        Args:
            ...
            expert (object): the expert that the policy is trying to imitate
    ...
    # ---------------------------------
    def generate_rollouts_for_imitation(self):
        self.reset_all_rollouts(test)
        ...
        # generate episodes
        obs, achieved_goals, acts, goals, successes = [], [], [], [], []
        info_values = [np.empty((self.T, 
                                 self.rollout_batch_size, 
                                 self.dims['info_' + key]), np.float32) 
                       for key in self.info_keys]
        Qs = []

        T_switch = np.random.choice(T)

        for t in range(self.T):
            if t >= T_switch:
                # Use the expert policy
                policy_output = self.expert.get_actions(
                    o, ag, self.g,
                    compute_Q=True,
                    noise_eps=0.0,
                    random_eps=0.0,
