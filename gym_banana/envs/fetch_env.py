...
class BlockEnv(robot_env.RobotPhysEnv):
    """Superclass for all Fetch environments.
    """
    ...
    def __init__(
        ...
        self.achieved_goal = [-1 for _ in range(NUM_BLOCKS ** 2)]
    ...
    def compute_reward(self, achieved_goal, goal, info):
        # Compute distance between goal and the achieved goal.
        d = np.sum(achieved_goal * goal)
        c = np.count_nonzero(goal)
    ...
    def _get_obs(self):
        ...
        for i in range(self.num_blocks):
            ...
            # gripper state
            temp_rel_pos = object_pos - grip_pos
            temp_velp -= grip_velp
    ...
    def _is_success(self, achieved_goal, desired_goal):
        r = compute_reward(achieved_goal, desired_goal, None)
        return r == 0
